...
sqldb.sequelize.sync({ force: false })
  .then(function() {
    return sqldb.Game.Quote.destroy({ force: force, where: {} })
  })
  .then(function() {
    return sqldb.Game.Showcase.destroy({ force: force, where: {} });
  })
  .then(function() {
    return sqldb.Game.destroy({ force: force, where: {} });
  })
  .then(function() {
    for (var shortname in games) {
      (function(game, shortname) {
        game.short_name = shortname;
        game.screenshots = (game.captures || []).length
        sqldb.Game.create(game).then(function(game_obj) {
          game.quotes = game.quotes || [];
          game.quotes.forEach(function(quote) {
            game_obj.createQuote({
              quote: quote.quote,
              author_name: quote.author.name,
              author_link: quote.author.link,
              author_tag: quote.author.tag
            });
          });

          game.showcase = game.showcase || [];
          game.showcase.forEach(function(showcase) {
            game_obj.createShowcase({
              link: showcase.link,
              text: showcase.text,
